...
## Pillar handoffs to Claude Code (open queue as of 2026-06-27)

**UPDATE 2026-06-27 (Saturday):** No change to pillar status — `public/compliance/` still 404s. The patch surface is now confirmed exhausted for the **4th consecutive run** (6/23, 6/26, 6/27 holds + 6/25 shipped-the-last-staged). The single citation win (admt ~pos 5) and the entire 20% compliance rate ride on ONE blog post. **The only growth lever left for this property is pillars + API keys** — both outside this optimizer's whitelist. Standing recommendation sharpened: dispatch `/compliance/ai-policy-template` first (we already own the 3 templates it needs).

| Query ID | Target page needed | Citation demand | Priority | Notes |
|----------|--------------------|----------------:|----------|-------|
| ai-policy-template-smb | /compliance/ai-policy-template | high | **P1 — BUILD FIRST** | Winners rank purely on free one-click downloadable templates. We ALREADY have 3 editable templates gated behind /compliance-assessment. A pillar surfaces them to this query AND is the Engine-1 conversion mechanism. Lowest-effort, highest-conversion open pillar. |
| colorado-ai-act-small-business | /compliance/colorado-ai-act | high | **P0** | Blog swings in/out of top-10 (pos 2 on 6/22; out 6/23, 6/26, 6/27). Single-page volatility; pillar 404s. availablelaw.com now owns SMB framing (new target_competitor). Carry reconciled <=40-employee employer exemption + 60-day-cure accuracy. |
| eu-ai-act-small-business | /compliance/eu-ai-act | high | P0 | Article 50 transparency live Aug 2 2026; high-risk possibly delayed to Dec 2 2027 (Digital Omnibus). hklaw/legalnodes/colombani/digitalapplied/bakermckenzie rank. Pillar should carry the dual-timeline nuance. |
| ai-compliance-assessment | /compliance-assessment | — | **DONE 2026-06-15** | Page LIVE & fully optimized (re-verified in full 6/27 — owns free/5-min/no-signup framing). 0 citations through 6/27 (day 12/14) = indexing latency. Bucket-failure threshold fires 6/29 if still 0%. |
| ai-risk-assessment-tool | /compliance-assessment | — | **DONE 2026-06-15** | Same page; riskhelper.ai owns 5-minute no-account framing (~pos 2, 6/27) — which our page already matches verbatim. Monitor. |
| msp-ai-compliance-resell | /partners/msp | medium-high | P2 | Engine 2. "AI governance audit" is the recurring high-margin wedge across all winners. lumenova/channelpro/connectwise/integris/worksent/guardz rank. Zero MSP content on-site. |
...
## Competitor & accuracy watch (updated 2026-06-27)

- **availablelaw.com (PROMOTED to target_competitor 2026-06-27):** owns the colorado-ai-act-small-business SMB-rewrite framing across 3 measured runs (6/23, 6/26, 6/27). Now the target_competitor on that query, replacing the stale repealed SB 24-205 bill-page URL.
- **digitalapplied.com (CROSSED 3+ THRESHOLD AGAIN 2026-06-27):** appeared on 4 query-result sets (eu-ai-act, ai-compliance-checklist, openai-partner-for-consultants, openai-partner-network-tiers). Already the target_competitor on eu-ai-act; all queries it appeared on have a target_competitor set -> no seed write (preserve rule). Durable cross-bucket competitor (partner + compliance).
...
- **OpenAI Partner Network (surface):** openai.com #1 on both partner queries 6/27. Our /openai-smb-partner + llms.txt carry the terminology (shipped 6/25) — still indexing, no rank movement yet.
- **glacis.io:** remains ~pos 10 on colorado-sb-26-189-admt 6/27 (below sayfeai ~pos 5). Our blog holds above it.
...
- **riskhelper.ai (target_competitor):** ~pos 2 for ai-risk-assessment-tool; owns 5-minute no-account framing — which /compliance-assessment already matches verbatim.
...
- **penncomp.com / ademero.com:** #1 / #2 (ademero = target_competitor) for ai-consultant-houston 6/27. /about still lacks local-page structure.
- **P0 ACCURACY — Colorado SB 26-189 small-business exemption: RESOLVED + FULLY SHIPPED 2026-06-17.** No general small-business carve-out; narrow employer-specific exception for employers with 40 or fewer employees (down from 50 under SB 24-205); replacement adds a 60-day right to cure. Reconciled across llms.txt, llms-full.txt, blog, ADMT FAQ #10. Re-verified consistent (6/26 blog read). No open accuracy items.

## Deferred / queued patches — STATUS

| File | Patch type | Status |
|------|-----------|-------------------------|
| (none) | — | **PATCH QUEUE EMPTY (confirmed 2026-06-27, 4th straight run).** No staged patches. /compliance-assessment re-verified comprehensive; no new diagnosable gap. Remaining gains are pillars + API keys. |
...
| public/blog/ai-compliance-colorado-eu-small-business-2026.html | ADMT definition FAQ #10 + freshness 2026-06-22 | **SHIPPED 2026-06-22.** VALIDATED (admt query holds ~pos 5 on 6/27). |
...
- **PATCH QUEUE EMPTY (2026-06-27).** Confirmed exhausted 4th straight run. /compliance-assessment re-read in full and confirmed it owns the winning framing — no patchable gap exists. Remaining durable gains are pillars + API keys, not patches.
- **#1 LEVER = PILLARS, NOT PATCHES.** public/compliance/ still does not exist. Build order: **(1) /compliance/ai-policy-template [BUILD FIRST — we already own the 3 templates it needs]**, (2) /compliance/colorado-ai-act, (3) /compliance/eu-ai-act; then (4) /partners/msp, (5) /houston local page. Claude Code's domain. The entire 20% compliance rate rides on ONE blog post.
- **ASSESSMENT BUCKET — threshold fires 6/29:** /compliance-assessment live since 6/15, 0 citations through 6/27 (day 12/14). If still 0% on 2026-06-29 (day 14), fire the "Assessment bucket is failing — strategy review needed" alert. Expected conclusion: indexing latency on a fully-optimized page (re-verified 6/27), but honor the threshold.
...
- **ADMT WIN — monitor:** colorado-sb-26-189-admt held ~pos 5 (6/27, 3rd straight). Watch for law-firm re-entry (workforcebulletin/finnegan).
- **#2 LEVER — ADD API KEYS:** OPENAI / ANTHROPIC / SERPAPI / PERPLEXITY. Every number in this ledger remains a directional WebSearch proxy until then (35 proxy-only runs). At a flat ~1/12 with the patch surface exhausted, this is now the highest-value unblock the optimizer can receive.
...
- **2026-06-27 (Saturday):** 1/12 (8.3%). admt HELD ~pos 5 (3rd straight; glacis ~pos 10 below). **/compliance-assessment re-read in full — owns the winning free/5-min/no-signup framing; assessment loss is indexing latency, not content.** 0 pages shipped (disciplined hold; patch surface exhausted 4th straight run). SEED: availablelaw.com promoted to target_competitor on colorado-ai-act-small-business (replaced stale repealed SB 24-205 URL). Assessment day 12/14. Single push (ledger -> 2nd commit, file size).
...
- **Bucket-failure rule (v2):** fired 2026-05-30; cleared. Compliance 20% (1/5); Assessment 0% (day 12/14 since live — threshold fires 6/29 if still 0%); Partner 0% (pillar/asset-gated; Partner Network page patches shipped 6/25, still indexing); Local 0% (structural, gated on local page).
- **Single-push rule (v2.1):** VIOLATED 6/16, 6/17, 6/19, 6/22, 6/25, 6/26 (file size). HONORED 6/23; 6/27 split to 2 commits (inline-content size). Standing recommendation: API-based push script to enable large single-call commits.
